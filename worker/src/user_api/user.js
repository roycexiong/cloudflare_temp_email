...
import { checkCfTurnstile, getJsonSetting } from "../utils"
...
export default {
    verifyCode: async (c) => {
        ...
        if (settings.enableMailAllowList
            ...
            return c.text("Invalid mail domain", 400)
        }
        ...
        // save to KV
        await c.env.KV.put(`temp-mail:${email}`, code, { expirationTtl: 120 });
        // send code to email
        try {
    ...
    register: async (c) => {
        ...
        // check enable
        if (!settings.enable) {
            return c.text("Third party user disabled", 403)
        }
        // check request
